...
Track::Ptr Factory::getTrackBySdp(const SdpTrack::Ptr &track) {
    ...
    std::cout<<"track->_codec.data()"<< track->_codec.data()<<std::endl;
    ...
    if (strcasecmp(track->_codec.data(), "L16") == 0) {
        return std::make_shared<G711Track>(CodecL16, track->_samplerate, track->_channel, 16);
    }
    ...
    WarnL << track->_codec;
    WarnL << track->_samplerate;    
    WarnL << track->_channel;
...
RtpCodec::Ptr Factory::getRtpDecoderByTrack(const Track::Ptr &track) {
    switch (track->getCodecId()){
        ...
        case CodecL16 : return std::make_shared<CommonRtpDecoder>(track->getCodecId());
...
RtmpCodec::Ptr Factory::getRtmpCodecByTrack(const Track::Ptr &track, bool is_encode) {
    switch (track->getCodecId()){
        ...
        case CodecG711U : {
            ...
            if (is_encode && (audio_track->getAudioSampleRate() != 16000 ||
        ...
        case CodecL16:{
            auto audio_track = dynamic_pointer_cast<AudioTrack>(track);
            if (is_encode && (audio_track->getAudioSampleRate() != 44100 ||
                              audio_track->getAudioChannel() != 1 ||
                              audio_track->getAudioSampleBit() != 16)) {
                //rtmp对g711只支持8000/1/16规格，但是ZLMediaKit可以解析其他规格的G711
                WarnL << "RTMP只支持44100/1/16规格的L16,目前规格是:"
                      << audio_track->getAudioSampleRate() << "/"
                      << audio_track->getAudioChannel() << "/"
                      << audio_track->getAudioSampleBit()
                      << ",该音频已被忽略";
